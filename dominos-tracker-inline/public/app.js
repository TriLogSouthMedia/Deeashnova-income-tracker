...
function initCharts() {
  ...
  cExp = new Chart(document.getElementById('cExp'), {
    type: 'doughnut',
    data: { labels: ['Fuel','Grocery','Phone','WiFi','Food','Maint','Insur','Misc','Other'], datasets: [{ data: [], backgroundColor: ['#ff1744','#ff9100','#2962ff','#e040fb','#ec4899','#6366f1','#00bfa5','#f97316','#64748b'], borderWidth: 0 }] },
...
═══ SHIFT HIGHLIGHTS ═══
Best Performing Day: ${bestDay.date} ($${bestDay.daily_profit.toFixed(2)} profit)
Worst Performing Day: ${worstDay.date} ($${worstDay.daily_profit.toFixed(2)} profit)
...
async function exportPDF() {
  ...
  try {
    ...
    let tableData = entries.map(e => [
      ...
      e.km, e.deliveries, '$' + (e.fuel||0).toFixed(2), '$' + e.daily_profit.toFixed(2)
    ]);

    doc.autoTable({
      startY: 25,
      head: [['Date', 'In', 'Out', 'Hrs', 'Gross', 'Tax', 'Net', 'KM', 'Deliv', 'Fuel', 'Profit']],
      body: tableData,
      theme: 'grid',
