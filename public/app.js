...
async function handleLogin(e) {
  ...
  try {
    ...
    if (data.success) {
      ...
      document.getElementById('loginError').textContent = data.error || 'Invalid email or password';
    }
  } catch (err) {
...
async function handleRegister(e) {
  ...
  if (!emailRegex.test(username)) {
    document.getElementById('registerError').textContent = 'Please enter a valid email address';
    return;
  }
...
function recalcEntry(e) {
  ...
  if (hours > settings.ot) {
    ...
    regularPay = settings.ot * settings.rate;
    otPay = otHours * settings.rate * settings.otm;
    gross = regularPay + otPay;
    e.has_overtime = true;
  } else {
    regularPay = hours * settings.rate;
    otPay = 0;
    gross = regularPay;
    e.has_overtime = false;
  }

  e.regular_pay = parseFloat(regularPay.toFixed(2));
  e.ot_pay = parseFloat(otPay.toFixed(2));
...
function render() {
  ...
  filtered.forEach(e => {
    ...
    tr.innerHTML = `
      ...
      <td class="cell-calc">${sym}${e.regular_pay.toFixed(2)}</td>
      <td class="cell-calc" style="color:var(--warning)">${e.has_overtime ? sym + e.ot_pay.toFixed(2) : '-'}</td>
      <td class="cell-calc" style="font-weight:800">${sym}${e.gross.toFixed(2)}</td>
  ...
  let wRegularPay = weekEntries.reduce((s,e) => s + (e.regular_pay || 0), 0);
  let wOTPay = weekEntries.reduce((s,e) => s + (e.ot_pay || 0), 0);
  ...
  document.getElementById('wRegular').textContent = sym + wRegularPay.toFixed(2);
  document.getElementById('wOTPay').textContent = sym + wOTPay.toFixed(2);
  ...
  let mRegularPay = monthEntries.reduce((s,e) => s + (e.regular_pay || 0), 0);
  let mOTPay = monthEntries.reduce((s,e) => s + (e.ot_pay || 0), 0);
  ...
  document.getElementById('mRegular').textContent = sym + mRegularPay.toFixed(2);
  document.getElementById('mOTPay').textContent = sym + mOTPay.toFixed(2);
...
function previewPhoto(input) {
  if (input.files && input.files[0]) {
    ...
    reader.onload = function(e) {
      ...
      let placeholder = document.getElementById('profPhotoPlaceholder');
...
async async function saveProfile() {
  ...
  try {
    if (photoInput.files && photoInput.files[0]) {
      let reader = new FileReader();
      let photoData = await new Promise((resolve) => {
...
async function doSaveProfile(data) {
  try {
    ...
    if (result.success) {
      ...
      let displayName = currentUser.name || currentUser.email || currentUser.username || 'Guest';
      document.getElementById('userBadge').textContent = '👤 ' + displayName;
      // Update photo preview if saved
      if (data.profile_photo) {
        let preview = document.getElementById('profPhotoPreview');
        let placeholder = document.getElementById('profPhotoPlaceholder');
        if (preview) { preview.src = data.profile_photo; preview.style.display = 'block'; }
        if (placeholder) placeholder.style.display = 'none';
      }
      hideProfile();
      showT('Profile saved!');
    } else {
      showT(result.error || 'Failed to save profile', true);
    }
  } catch (err) {
