...
async function sendEmailReport() {
  ...
  try {
    ...
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    let sym = currencySymbols[currency] || '$';
    let pageWidth = doc.internal.pageSize.getWidth();   // 210mm
    let pageHeight = doc.internal.pageSize.getHeight(); // 297mm
    let margin = 12;
    let contentWidth = pageWidth - (margin * 2);
    ...
    function drawKPICard(x, y, w, h, iconColor, iconBg, label, value, subLabel, subValue) {
      ...
      doc.setFont('helvetica', 'bold');
      // Simple icon char
      let iconChar = label.includes('Income') ? '+' : label.includes('Expense') ? '-' : label.includes('Profit') ? '$' : label.includes('Hour') ? 'H' : label.includes('Savings') ? 'S' : '$';
    ...
    function drawSparkline(x, y, w, h, data, color, fill) {
      ...
      for (let i = 0; i < data.length - 1; i++) {
        let x1 = x + i * stepX;
        let y1 = y + h - ((data[i] - min) / range) * h;
        let x2 = x + (i + 1) * stepX;
        let y2 = y + h - ((data[i + 1] - min) / range) * h;
        drawLine(x1, y1, x2, y2);
      }

      if (fill) {
        setFill(color[0], color[1], color[2]);
        let lastX = x + (data.length - 1) * stepX;
        let lastY = y + h - ((data[data.length - 1] - min) / range) * h;
    ...
    let healthScore = Math.min(100, Math.max(0, totalProfit > 0 ? 70 + (totalProfit / totalGross) * 30 : 30 + (totalProfit / totalGross) * 20));
    let healthLabel = healthScore >= 71 ? 'GOOD' : healthScore >= 41 ? 'FAIR' : 'NEEDS ATTENTION';
    let healthColor = healthScore >= 71 ? C.green : healthScore >= 41 ? C.orange : C.red;
    ...
    let profitTrend = sortedEntries.map(e => e.daily_profit);
    ...
    doc.setFontSize(5.5);
    doc.setFont('helvetica', 'normal');
    let healthMsg = healthScore >= 71 ? 'You are in a good financial position. Keep optimizing!' : 
                    healthScore >= 41 ? 'Your finances need some attention. Review expenses.' :
                    'Critical: Immediate action needed on your finances.';
    let msgLines = doc.splitTextToSize(healthMsg, leftColW - 10);
    doc.text(msgLines, gaugeX, gaugeY + 20, { align: 'center' });

    // --- KEY PERFORMANCE OVERVIEW (Right Column - 6 cards in 2x3 grid) ---
    let rightX = leftX + leftColW + 5;
    let rightW = contentWidth - leftColW - 5;
    let cardW = (rightW - 10) / 3;
    let cardH = 22;
    ...
    let sparkData = [incomeTrend, expenseTrend, profitTrend, 
    ...
    kpis.forEach((kpi, i) => {
      let cx = rightX + (i % 3) * (cardW + 3);
      let cy = y + 7 + Math.floor(i / 3) * (cardH + 3);
      ...
      doc.setFontSize(6);
      doc.setFont('helvetica', 'bold');
      let iconChar = kpi.label.includes('Income') ? '+' : kpi.label.includes('Expense') ? '-' : kpi.label.includes('Profit') ? '$' : kpi.label.includes('Hour') ? 'H' : kpi.label.includes('Savings') ? 'S' : '$';
      doc.text(iconChar, cx + 6, cy + cardH/2 + 1.5, { align: 'center' });
    ...
    let barW = 22;
    let barBaseY = chart1Y + chartH - 10;
    ...
    // Insight alert
    let diffAmount = Math.abs(totalProfit);
    let diffText = totalProfit >= 0 
    ...
    if (incomeTrend.length >= 2) {
      ...
      let stepXI = lineW / (incomeTrend.length - 1);
      setDraw(C.green[0], C.green[1], C.green[2]);
      doc.setLineWidth(0.5);
      for (let i = 0; i < incomeTrend.length - 1; i++) {
        let px1 = lineX + i * stepXI;
        let py1 = lineY + lineH - ((incomeTrend[i] - minI) / rangeI) * lineH;
        let px2 = lineX + (i + 1) * stepXI;
        let py2 = lineY + lineH - ((incomeTrend[i + 1] - minI) / rangeI) * lineH;
        drawLine(px1, py1, px2, py2);
      }
      // Dots
      setFill(C.green[0], C.green[1], C.green[2]);
      incomeTrend.forEach((v, i) => {
        let px = lineX + i * stepXI;
        let py = lineY + lineH - ((v - minI) / rangeI) * lineH;
    ...
    if (expenseTrend.length >= 2) {
      ...
      let stepXE = lineW / (expenseTrend.length - 1);
      setDraw(C.red[0], C.red[1], C.red[2]);
      doc.setLineWidth(0.5);
      for (let i = 0; i < expenseTrend.length - 1; i++) {
        let px1 = lineX + i * stepXE;
        let py1 = lineY + lineH - ((expenseTrend[i] - minE) / rangeE) * lineH;
        let px2 = lineX + (i + 1) * stepXE;
        let py2 = lineY + lineH - ((expenseTrend[i + 1] - minE) / rangeE) * lineH;
        drawLine(px1, py1, px2, py2);
      }
      setFill(C.red[0], C.red[1], C.red[2]);
      expenseTrend.forEach((v, i) => {
        let px = lineX + i * stepXE;
        let py = lineY + lineH - ((v - minE) / rangeE) * lineH;
        drawCircle(px, py, 1, 'F');
      });
    }
    ...
    let donutCX = donutX + 28;
    let donutCY = donutY + 32;
    let donutR = 14;
    let innerR = 7;
    ...
    if (totalExp > 0 && expBreakdown.length > 0) {
      ...
      expBreakdown.forEach((exp, i) => {
        ...
        let segments = Math.max(3, Math.floor(angle * 10));
        setFill(exp.color[0], exp.color[1], exp.color[2]);
        for (let j = 0; j < segments; j++) {
          let a1 = startAngle + (angle / segments) * j;
          let a2 = startAngle + (angle / segments) * (j + 1);
          let x1 = donutCX + Math.cos(a1) * donutR;
          let y1 = donutCY + Math.sin(a1) * donutR;
          let x2 = donutCX + Math.cos(a2) * donutR;
          let y2 = donutCY + Math.sin(a2) * donutR;
    ...
    let legendX = donutX + 50;
    let legendStartY = donutY + 14;
    expBreakdown.slice(0, 5).forEach((exp, i) => {
      let ly = legendStartY + i * 8;
      setFill(exp.color[0], exp.color[1], exp.color[2]);
    ...
    insights.forEach((ins, i) => {
      ...
      doc.setFontSize(6);
      doc.setFont('helvetica', 'normal');
      let lines = doc.splitTextToSize(ins.text, insightW - 20);
    ...
    let summaryW = contentWidth * 0.52;
    let summaryX = margin;
    let summaryH = 55;
    ...
    doc.setFontSize(6.5);
    doc.setFont('helvetica', 'normal');
    let summaryText = `During ${monthYear}, you completed ${totalShifts} work shift${totalShifts > 1 ? 's' : ''} totaling ${totalHours.toFixed(2)} hours.\n\nYour total earnings reached ${sym}${totalGross.toFixed(2)}, while total expenses amounted to ${sym}${totalExp.toFixed(2)}.\n\nThis resulted in a net ${totalProfit >= 0 ? 'profit' : 'loss'} of ${sym}${Math.abs(totalProfit).toFixed(2)} for the month.\n\nAlthough income remained steady, expenses ${totalExp > totalGross ? 'significantly outweighed' : 'were managed within'} earnings. The primary contributors were ${expBreakdown[0] ? expBreakdown[0].name.toLowerCase() : 'fuel'} and personal spending categories.\n\nThe report suggests ${totalProfit < 0 ? 'increasing work hours or reducing discretionary spending' : 'maintaining current work patterns'} to improve profitability in future months.`;

    let summaryLines = doc.splitTextToSize(summaryText, summaryW - 12);
    doc.text(summaryLines, summaryX + 5, y + 20);

    // --- 2. INCOME ANALYSIS (Right) ---
    let incomeX = summaryX + summaryW + 5;
    let incomeW = contentWidth - summaryW - 5;
    let incomeH = 55;

    drawCard(incomeX, y, incomeW, incomeH, C.white, C.borderLight, 3);
    ...
    let trendX = incomeX + 5;
    let trendY = y + 15;
    let trendW = incomeW - 10;
    let trendH = 22;
    ...
    if (sortedEntries.length >= 2) {
      ...
      let stepXN = trendW / (netData.length - 1);
      ...
      for (let i = 0; i < netData.length; i++) {
        let px = trendX + i * stepXN;
        let py = trendY + trendH - ((netData[i] - minN) / rangeN) * trendH;
        path.push([px, py]);
      }
      // Draw filled polygon
      if (path.length > 1) {
        ...
        // Highlight best day
        let bestIdx = netData.indexOf(maxN);
        setFill(C.green[0], C.green[1], C.green[2]);
        ...
        doc.setFontSize(4);
        doc.setFont('helvetica', 'bold');
        let bestDate = new Date(sortedEntries[bestIdx].date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    ...
    setText(C.textLight[0], C.textLight[1], C.textLight[2]);
    doc.setFontSize(4);
    let yLabels = ['$250', '$200', '$150', '$100', '$50', '$0'];
    yLabels.forEach((l, i) => {
    ...
    let cardMiniW = (incomeW - 14) / 3;
    miniCards.forEach((card, i) => {
      let cx = incomeX + 5 + i * (cardMiniW + 2);
      let cy = y + incomeH - 14;
    ...
    y += 60;
    let expW = contentWidth * 0.48;
    let expX = margin;
    let expH = 45;

    drawCard(expX, y, expW, expH, C.white, C.borderLight, 3);
    ...
    expBreakdown.slice(0, 5).forEach((exp, i) => {
      ...
      if (pct > 0) {
        setFill(exp.color[0], exp.color[1], exp.color[2]);
        drawRoundedRect(expX + 3, rowY - 1, (expW - 6) * (pct / 100), 3, 1, 1, 'F');
      }
      ...
      setText(C.textLight[0], C.textLight[1], C.textLight[2]);
      doc.setFont('helvetica', 'normal');
      doc.text(pct + '%', expX + expW - 8, rowY + 3.5, { align: 'right' });
    });

    // --- 4. PRODUCTIVITY ANALYSIS (Right) ---
    let prodX = expX + expW + 5;
    let prodW = contentWidth - expW - 5;
    let prodH = 45;
    ...
    let metricW = (prodW - 10) / 5;
    prodMetrics.forEach((m, i) => {
      let mx = prodX + 5 + i * metricW;
      let my = y + 14;

      // Gauge arc
      let gaugeR = 8;
      let gaugePct = Math.min(100, Math.max(0, m.label.includes('Profit') && totalProfit < 0 ? 30 : 70 + Math.random() * 20));
    ...
    // 3 projection cards
    let projMultiplier = 4.3; // Monthly projection from weekly data
    let projections = [
    ...
    let projCardW = (contentWidth - 20) / 3;
    projections.forEach((proj, i) => {
      let px = margin + 5 + i * (projCardW + 3);
      let py = y + 14;
    ...
    let futX = margin + 5;
    let futY = y + 32;
    let futW = contentWidth - 10;
    let futH = 4;

    // Simple trend line
    setDraw(C.green[0], C.green[1], C.green[2]);
    doc.setLineWidth(0.4);
    let futMonths = ['Jul 2026', 'Aug 2026', 'Sep 2026', 'Oct 2026', 'Nov 2026', 'Dec 2026'];
    let futIncome = [totalGross * 0.9, totalGross * 1.0, totalGross * 1.05, totalGross * 1.1, totalGross * 1.05, totalGross * 1.15];
    let futExpenses = [totalExp * 1.05, totalExp * 1.1, totalExp * 1.08, totalExp * 1.12, totalExp * 1.1, totalExp * 1.15];
    let futProfit = futIncome.map((inc, i) => inc - futExpenses[i]);

    let minFut = Math.min(...futProfit, ...futIncome, ...futExpenses);
    let maxFut = Math.max(...futProfit, ...futIncome, ...futExpenses);
    let rangeFut = maxFut - minFut || 1;
    let stepXFut = futW / (futMonths.length - 1);

    // Income line
    setDraw(C.green[0], C.green[1], C.green[2]);
    for (let i = 0; i < futIncome.length - 1; i++) {
      let px1 = futX + i * stepXFut;
      let py1 = futY - ((futIncome[i] - minFut) / rangeFut) * futH;
      let px2 = futX + (i + 1) * stepXFut;
      let py2 = futY - ((futIncome[i + 1] - minFut) / rangeFut) * futH;
      drawLine(px1, py1, px2, py2);
    }

    // Expense line
    setDraw(C.red[0], C.red[1], C.red[2]);
    for (let i = 0; i < futExpenses.length - 1; i++) {
      let px1 = futX + i * stepXFut;
      let py1 = futY - ((futExpenses[i] - minFut) / rangeFut) * futH;
      let px2 = futX + (i + 1) * stepXFut;
      let py2 = futY - ((futExpenses[i + 1] - minFut) / rangeFut) * futH;
      drawLine(px1, py1, px2, py2);
    }

    // Profit line
    setDraw(C.blue[0], C.blue[1], C.blue[2]);
    for (let i = 0; i < futProfit.length - 1; i++) {
      let px1 = futX + i * stepXFut;
      let py1 = futY - ((futProfit[i] - minFut) / rangeFut) * futH;
      let px2 = futX + (i + 1) * stepXFut;
      let py2 = futY - ((futProfit[i + 1] - minFut) / rangeFut) * futH;
      drawLine(px1, py1, px2, py2);
    }
