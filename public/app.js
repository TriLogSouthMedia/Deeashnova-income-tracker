  ...
  try {
    ...
    if (data.success) {
      ...
      try {
        ...
        document.getElementById('loginError').textContent = 'Login succeeded but app failed to load. Please refresh or check console.';
      }
    } else {
...
function recalcEntry(e) {
  ...
  // Dynamically calculate expenses from all personal expense fields
  let expenses = 0;
  if (fieldConfig && fieldConfig.personalExpenses) {
    fieldConfig.personalExpenses.forEach(field => {
      expenses += parseFloat(e[field.id]) || 0;
    });
  } else {
    // Fallback to hardcoded fields
    expenses = (parseFloat(e.fuel)||0) + (parseFloat(e.grocery)||0) + (parseFloat(e.phone)||0) + (parseFloat(e.wifi)||0) + (parseFloat(e.food)||0) + (parseFloat(e.maintenance)||0) + (parseFloat(e.insurance)||0) + (parseFloat(e.misc)||0) + (parseFloat(e.other)||0);
  }
  // Always include fuel in expenses (it's a work expense but still reduces profit)
  expenses += parseFloat(e.fuel) || 0;
...
async function addEntry() {
  ...
  // Dynamically build entry from all fields in fieldConfig
  let entry = {
    date: formData.date,
    time_in: timeIn,
    time_out: timeOut,
  ...
  // Add all work detail fields
  fieldConfig.workDetails.forEach(field => {
    if (field.id === 'date' || field.id === 'time_in' || field.id === 'time_out') return;
    let val = formData[field.id];
    if (field.type === 'number') {
      entry[field.id] = parseFloat(val) || 0;
    } else if (field.type === 'select') {
      entry[field.id] = val || (field.options ? field.options[0] : 'regular');
    } else {
      entry[field.id] = val || '';
    }
  });

  // Add all personal expense fields
  fieldConfig.personalExpenses.forEach(field => {
    let val = formData[field.id];
    entry[field.id] = parseFloat(val) || 0;
  });
...
function render() {
  ...
  let totalGross = 0, totalNet = 0, totalHours = 0, totalDeliv = 0, totalKM = 0, totalExp = 0, totalProfit = 0, totalTips = 0;
  filtered.forEach(e => {
    totalGross += e.gross;
    totalNet += e.net;
    totalHours += e.hours;
    totalDeliv += e.deliveries || 0;
    totalKM += e.km || 0;
    totalTips += e.tips || 0;
    let expSum = 0;
    if (fieldConfig && fieldConfig.personalExpenses) {
      fieldConfig.personalExpenses.forEach(field => {
        expSum += parseFloat(e[field.id]) || 0;
      });
    } else {
      expSum = (e.fuel||0) + (e.grocery||0) + (e.phone||0) + (e.wifi||0) + (e.food||0) + (e.maintenance||0) + (e.insurance||0) + (e.misc||0) + (e.other||0);
    }
    totalExp += expSum;
  ...
  let dTips = document.getElementById('dTips');
  if (dTips) dTips.textContent = sym + totalTips.toFixed(2);
  ...
  filtered.forEach(e => {
    ...
    if (fieldConfig && fieldConfig.personalExpenses) {
      fieldConfig.personalExpenses.forEach(field => {
        totalExp += parseFloat(e[field.id]) || 0;
      });
    } else {
      totalExp = (parseFloat(e.fuel)||0) + (parseFloat(e.grocery)||0) + (parseFloat(e.phone)||0) + (parseFloat(e.wifi)||0) + (parseFloat(e.food)||0) + (parseFloat(e.maintenance)||0) + (parseFloat(e.insurance)||0) + (parseFloat(e.misc)||0) + (parseFloat(e.other)||0);
    }
    // Fuel is a work expense but still counts toward total expenses
    totalExp += parseFloat(e.fuel) || 0;
    tr.innerHTML = `
      ...
      <td style="text-align:center">${e.km || 0}</td>
      <td class="delivCol" style="text-align:center">${e.deliveries || 0}</td>
      <td style="text-align:right">$${(e.tips || 0).toFixed(2)}</td>
      <td style="text-align:right;color:var(--danger);font-weight:600">$${(e.fuel || 0).toFixed(2)}</td>
  ...
  let wDeliv = weekEntries.reduce((s,e) => s + (e.deliveries||0), 0);
  let wKM = weekEntries.reduce((s,e) => s + (e.km||0), 0);
  let wFuel = weekEntries.reduce((s,e) => s + (e.fuel||0), 0);
  let wTips = weekEntries.reduce((s,e) => s + (e.tips||0), 0);
  let wExp = weekEntries.reduce((s,e) => {
    let expSum = 0;
    if (fieldConfig && fieldConfig.personalExpenses) {
      fieldConfig.personalExpenses.forEach(field => {
        expSum += parseFloat(e[field.id]) || 0;
    ...
    // Include fuel in total expenses
    expSum += parseFloat(e.fuel) || 0;
    return s + expSum;
  }, 0);
  ...
  let wTipsEl = document.getElementById('wTips');
  if (wTipsEl) wTipsEl.textContent = sym + wTips.toFixed(2);
  ...
  let mDeliv = monthEntries.reduce((s,e) => s + (e.deliveries||0), 0);
  let mKM = monthEntries.reduce((s,e) => s + (e.km||0), 0);
  let mFuel = monthEntries.reduce((s,e) => s + (e.fuel||0), 0);
  let mTips = monthEntries.reduce((s,e) => s + (e.tips||0), 0);
  let mExp = monthEntries.reduce((s,e) => {
    let expSum = 0;
    if (fieldConfig && fieldConfig.personalExpenses) {
      fieldConfig.personalExpenses.forEach(field => {
        expSum += parseFloat(e[field.id]) || 0;
  ...
  let mTipsEl = document.getElementById('mTips');
  if (mTipsEl) mTipsEl.textContent = sym + mTips.toFixed(2);
...
The report suggests ${totalProfit < 0 ? 'increasing work hours or reducing discretionary spending' : 'maintaining current work patterns'} to improve profitability in future months.`;
    ...
    for (i = 0; i < trendPoints.length - 1; i++) {
    ...
    expBreakdown.slice(0, 5).forEach((exp, i) => {
      let rowY = y + 22 + i * 5;
      pct = totalExp > 0 ? Math.round((exp.value / totalExp) * 100) : 0;
...
function renderViewModal(entry, editMode = false) {
  ...
  if (fieldConfig && fieldConfig.personalExpenses) {
    fieldConfig.personalExpenses.forEach(field => {
      let val = parseFloat(entry[field.id]) || 0;
      totalExp += val;
      if (val > 0) {
        let label = getLabel(field.id, field.label);
        expenseDetails += `<div class="view-item"><span class="view-item-label">${label}</span><span class="view-item-value negative">${sym}${val.toFixed(2)}</span></div>`;
      }
    });
  } else {
    // Fallback to hardcoded fields
    totalExp = (parseFloat(entry.fuel)||0) + (parseFloat(entry.grocery)||0) + (parseFloat(entry.phone)||0) + (parseFloat(entry.wifi)||0) + (parseFloat(entry.food)||0) + (parseFloat(entry.maintenance)||0) + (parseFloat(entry.insurance)||0) + (parseFloat(entry.misc)||0) + (parseFloat(entry.other)||0);
  ...
  if (!editMode) {
    ...
    // Edit mode - dynamically generate fields from fieldConfig
    let workDetailsHTML = '';
    fieldConfig.workDetails.forEach(field => {
      if (field.id === 'date' || field.id === 'time_in' || field.id === 'time_out') return;
      val = entry[field.id] || 0;
      let inputHTML = '';
      if (field.type === 'select') {
        inputHTML = `<select class="edit-select" id="edit_${field.id}">${field.options.map(o => `<option value="${o}" ${entry[field.id] === o ? 'selected' : ''}>${o.charAt(0).toUpperCase() + o.slice(1)}</option>`).join('')}</select>`;
      } else {
        inputHTML = `<input class="edit-input" id="edit_${field.id}" type="number" value="${val}" step="${field.step || '0.01'}">`;
      }
      workDetailsHTML += `<div class="view-item"><span class="view-item-label">${field.label}</span>${inputHTML}</div>`;
    });

    let expensesHTML = '';
    fieldConfig.personalExpenses.forEach(field => {
      val = entry[field.id] || 0;
      label = getLabel(field.id, field.label);
      expensesHTML += `<div class="view-item"><span class="view-item-label">${label}</span><input class="edit-input" id="edit_${field.id}" type="number" value="${val}" step="0.01"></div>`;
    });
...
async function saveEditMode() {
  ...
  // Read all dynamic work detail fields (includes tips, fuel, km, deliveries, etc.)
  fieldConfig.workDetails.forEach(field => {
    if (field.id === 'date' || field.id === 'time_in' || field.id === 'time_out') return;
    let el = document.getElementById('edit_' + field.id);
    if (el) {
      if (field.type === 'number') {
        entry[field.id] = parseFloat(el.value) || 0;
      } else if (field.type === 'select') {
  ...
  // Read all dynamic expense fields
  fieldConfig.personalExpenses.forEach(field => {
    el = document.getElementById('edit_' + field.id);
    if (el) entry[field.id] = parseFloat(el.value) || 0;
  });
...
function getFormData() {
  ...
  // Read notes separately (not in fieldConfig)
  let notesEl = document.getElementById('e_notes');
