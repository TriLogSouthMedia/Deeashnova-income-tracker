...
window.onerror = function(msg, url, line, col, error) {
  console.error('PAYPULSE ERROR:', msg, 'at line', line, 'col', col);
  alert('App Error: ' + msg + ' (line ' + line + ')');
  return false;
};
window.addEventListener('unhandledrejection', function(e) {
  console.error('PAYPULSE UNHANDLED PROMISE:', e.reason);
  alert('App Promise Error: ' + e.reason);
});
...
async function initApp() {
  ...
  showApp();
  console.log('[INIT] showApp called');
  ...
  console.log('[INIT] About to load shifts...');
  await loadShifts();
  console.log('[INIT] Shifts loaded, initCharts...');
  initCharts();
...
async function handleLogin(e) {
  ...
  try {
    console.log('[LOGIN] Sending fetch to /api/login');
    ...
    if (data.success) {
      console.log('[LOGIN] Success! Setting currentUser...');
      ...
      currency = currentUser.currency || 'CAD';
      disabledFields = JSON.parse(currentUser.disabled_fields || '[]');
      console.log('[LOGIN] Calling initApp...');
      await initApp();
      console.log('[LOGIN] initApp completed!');
    } else {
      console.log('[LOGIN] Server error:', data.error);
...
The report suggests ${totalProfit < 0 ? 'increasing work hours or reducing discretionary spending' : 'maintaining current work patterns'} to improve profitability in future months.`;
    ...
    expBreakdown.slice(0, 5).forEach((exp, i) => {
      ...
      doc.text(rowPct + '%', margin + 65, rowY + 2, { align: 'right' });
    });
...
// ==================== PREMIUM EMAIL PDF REPORT ====================
// Light theme based on the design layout
// Stage-built implementation

async function sendEmailReport() {
  var email = document.getElementById('emailExportAddress').value.trim();
  var includeReport = document.getElementById('emailIncludeReport').checked;
  ...
  try {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    var sym = currencySymbols[currency] || '$';
    var pageWidth = doc.internal.pageSize.getWidth();   // 210mm
    var pageHeight = doc.internal.pageSize.getHeight(); // 297mm
    var margin = 12;
    var contentWidth = pageWidth - (margin * 2);

    // ============ COLOR PALETTE (from design) ============
    const C = {
      headerBlue: [13, 71, 161],      // Deep blue header #0D47A1
      headerBlueLight: [25, 118, 210], // Lighter blue accent #1976D2
      white: [255, 255, 255],
      black: [0, 0, 0],
      textDark: [33, 33, 33],         // #212121
      textGray: [97, 97, 97],         // #616161
      textLight: [158, 158, 158],     // #9E9E9E
      bgWhite: [255, 255, 255],
      ...
      orangeLight: [255, 237, 213],   // #FFEDD5
      blue: [59, 130, 246],           // #3B82F6
      blueLight: [219, 234, 254],     // #DBEAFE
      purple: [139, 92, 246],         // #8B5CF6
    ...
    function drawRect(x, y, w, h, style) { doc.rect(x, y, w, h, style); }
    function drawRoundedRect(x, y, w, h, rx, ry, style) { doc.roundedRect(x, y, w, h, rx, ry, style); }
    function drawLine(x1, y1, x2, y2) { doc.line(x1, y1, x2, y2); }
    function drawCircle(x, y, r, style) { doc.circle(x, y, r, style); }

    // Draw a rounded card with subtle shadow effect
    function drawCard(x, y, w, h, fillColor, borderColor, radius) {
      radius = radius || 3;
      // Shadow (offset slightly)
      setFill(200, 200, 200);
      drawRoundedRect(x + 0.5, y + 0.5, w, h, radius, radius, 'F');
      // Actual card
      setFill(fillColor[0], fillColor[1], fillColor[2]);
      setDraw(borderColor[0], borderColor[1], borderColor[2]);
      drawRoundedRect(x, y, w, h, radius, radius, 'FD');
    }

    // Draw KPI card with icon circle
    function drawKPICard(x, y, w, h, iconColor, iconBg, label, value, subLabel, subValue) {
      drawCard(x, y, w, h, C.white, C.borderLight, 3);
      // Icon circle
      var cx = x + 8;
      var cy = y + h/2;
      setFill(iconBg[0], iconBg[1], iconBg[2]);
      drawCircle(cx, cy, 5, 'F');
      setText(iconColor[0], iconColor[1], iconColor[2]);
      doc.setFontSize(7);
      doc.setFont('helvetica', 'bold');
      // Simple icon char
      var iconChar = label.includes('Income') ? '+' : label.includes('Expense') ? '-' : label.includes('Profit') ? '$' : label.includes('Hour') ? 'H' : label.includes('Savings') ? 'S' : '$';
      doc.text(iconChar, cx, cy + 1.5, { align: 'center' });
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text(value, x + 16, y + 14);

      // Sub label
      if (subLabel) {
        setText(C.textLight[0], C.textLight[1], C.textLight[2]);
        doc.setFontSize(5);
        doc.setFont('helvetica', 'normal');
        doc.text(subLabel, x + 16, y + 19);
      }
      if (subValue) {
        setText(subValue >= 0 ? C.green[0] : C.red[0], subValue >= 0 ? C.green[1] : C.red[1], subValue >= 0 ? C.green[2] : C.red[2]);
        doc.setFontSize(5);
        doc.setFont('helvetica', 'bold');
        var arrow = subValue >= 0 ? '\u2191' : '\u2193';
        doc.text(arrow + ' ' + Math.abs(subValue) + '%', x + 30, y + 19);
      }
    }

    // Draw a mini sparkline (simple line chart)
    function drawSparkline(x, y, w, h, data, color, fill) {
      ...
      setDraw(color[0], color[1], color[2]);
      doc.setLineWidth(0.4);

      for (var i = 0; i < data.length - 1; i++) {
        var x1 = x + i * stepX;
        var y1 = y + h - ((data[i] - min) / range) * h;
        var x2 = x + (i + 1) * stepX;
        var y2 = y + h - ((data[i + 1] - min) / range) * h;
        drawLine(x1, y1, x2, y2);
      }

      if (fill) {
        setFill(color[0], color[1], color[2]);
        var lastX = x + (data.length - 1) * stepX;
        var lastY = y + h - ((data[data.length - 1] - min) / range) * h;
        drawCircle(lastX, lastY, 1.2, 'F');
      }
    }

    // Draw gauge arc (semi-circle)
    function drawGauge(x, y, radius, percentage, color, bgColor) {
      // Background arc (gray)
      setDraw(bgColor[0], bgColor[1], bgColor[2]);
      doc.setLineWidth(3);
      doc.arc(x, y, radius, Math.PI, 0, 'S');

      // Value arc
      var endAngle = Math.PI + (Math.PI * (percentage / 100));
      setDraw(color[0], color[1], color[2]);
      doc.arc(x, y, radius, Math.PI, endAngle, 'S');
    }

    // Draw horizontal bar
    function drawBar(x, y, w, h, percentage, color, bgColor) {
      setFill(bgColor[0], bgColor[1], bgColor[2]);
      drawRoundedRect(x, y, w, h, h/2, h/2, 'F');
      if (percentage > 0) {
        setFill(color[0], color[1], color[2]);
        drawRoundedRect(x, y, w * (percentage / 100), h, h/2, h/2, 'F');
      }
    }

    // ============ CALCULATIONS ============
    var totalGross = entries.reduce((s, e) => s + e.gross, 0);
    var totalNet = entries.reduce((s, e) => s + e.net, 0);
    var totalHours = entries.reduce((s, e) => s + e.hours, 0);
    var totalProfit = entries.reduce((s, e) => s + e.daily_profit, 0);
    var totalTips = entries.reduce((s, e) => s + (e.tips || 0), 0);
    var totalKM = entries.reduce((s, e) => s + e.km, 0);
    var totalDeliv = entries.reduce((s, e) => s + e.deliveries, 0);
    var totalShifts = entries.length;
    var avgHourly = totalHours > 0 ? totalNet / totalHours : 0;
    var avgProfit = entries.length > 0 ? totalProfit / entries.length : 0;
    var otCount = entries.filter(e => e.has_overtime).length;

    // Expense breakdown
    var expBreakdown = [
      { name: 'Fuel', value: entries.reduce((s, e) => s + (e.fuel || 0), 0), color: C.red, bg: C.redLight },
      { name: 'Grocery', value: entries.reduce((s, e) => s + (e.grocery || 0), 0), color: C.orange, bg: C.orangeLight },
      { name: 'Phone', value: entries.reduce((s, e) => s + (e.phone || 0), 0), color: C.blue, bg: C.blueLight },
      { name: 'WiFi', value: entries.reduce((s, e) => s + (e.wifi || 0), 0), color: C.purple, bg: C.purpleLight },
      { name: 'Food', value: entries.reduce((s, e) => s + (e.food || 0), 0), color: C.pink, bg: [252, 231, 243] },
      { name: 'Maintenance', value: entries.reduce((s, e) => s + (e.maintenance || 0), 0), color: C.teal, bg: [204, 251, 241] },
      { name: 'Insurance', value: entries.reduce((s, e) => s + (e.insurance || 0), 0), color: C.cyan, bg: [207, 250, 254] },
      { name: 'Misc', value: entries.reduce((s, e) => s + (e.misc || 0), 0), color: C.yellow, bg: [254, 249, 195] },
      { name: 'Other', value: entries.reduce((s, e) => s + (e.other || 0), 0), color: [148, 163, 184], bg: [241, 245, 249] }
    ].filter(e => e.value > 0).sort((a, b) => b.value - a.value);

    var totalExp = expBreakdown.reduce((s, e) => s + e.value, 0);

    // Health score
    var healthScore = Math.min(100, Math.max(0, totalProfit > 0 ? 70 + (totalProfit / totalGross) * 30 : 30 + (totalProfit / totalGross) * 20));
    var healthLabel = healthScore >= 71 ? 'GOOD' : healthScore >= 41 ? 'FAIR' : 'NEEDS ATTENTION';
    var healthColor = healthScore >= 71 ? C.green : healthScore >= 41 ? C.orange : C.red;

    // Date info
    var now = new Date();
    var monthYear = now.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
    var reportId = 'PP-' + now.getFullYear() + '-' + String(now.getMonth() + 1).padStart(2, '0') + '-' + String(now.getDate()).padStart(2, '0') + '-001';
    var genDate = now.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    var userName = currentUser ? (currentUser.name || currentUser.username) : 'Guest';

    // Trend data (for sparklines)
    var sortedEntries = entries.slice().sort((a, b) => new Date(a.date) - new Date(b.date));
    var incomeTrend = sortedEntries.map(e => e.gross);
    var expenseTrend = sortedEntries.map(e => (e.fuel || 0) + (e.grocery || 0) + (e.phone || 0) + (e.wifi || 0) + (e.food || 0));
    var profitTrend = sortedEntries.map(e => e.daily_profit);

    // ============ PAGE 1 ============

    // --- HEADER BAR ---
    setFill(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    drawRect(0, 0, pageWidth, 22, 'F');

    // Logo "P" icon
    setFill(C.white[0], C.white[1], C.white[2]);
    drawRoundedRect(margin, 5, 14, 12, 2, 2, 'F');
    setText(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('P', margin + 7, 13, { align: 'center' });

    // PayPulse title
    setText(C.white[0], C.white[1], C.white[2]);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text('PAYPULSE', margin + 18, 11);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'normal');
    doc.text('Monthly Financial Performance Report', margin + 18, 17);

    // Right side - Prepared for
    setText(C.white[0], C.white[1], C.white[2]);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'normal');
    doc.text('Prepared for', pageWidth - margin - 50, 9, { align: 'right' });
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text(userName, pageWidth - margin - 50, 16, { align: 'right' });

    // --- SUB-HEADER BAR ---
    var subY = 22;
    setFill(C.white[0], C.white[1], C.white[2]);
    drawRect(0, subY, pageWidth, 10, 'F');
    setDraw(C.borderLight[0], C.borderLight[1], C.borderLight[2]);
    drawLine(0, subY + 10, pageWidth, subY + 10);

    // Month badge
    setFill(C.blueLight[0], C.blueLight[1], C.blueLight[2]);
    drawRoundedRect(margin, subY + 2, 30, 6, 2, 2, 'F');
    setText(C.blue[0], C.blue[1], C.blue[2]);
    doc.setFontSize(6);
    doc.setFont('helvetica', 'bold');
    doc.text(monthYear, margin + 15, subY + 6, { align: 'center' });

    // Generated date
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(6);
    doc.setFont('helvetica', 'normal');
    doc.text('Generated: ' + genDate, margin + 38, subY + 6);

    // Report ID
    doc.text('Report ID: ' + reportId, pageWidth - margin, subY + 6, { align: 'right' });
    ...
    // Card background
    drawCard(leftX, y, leftColW, 52, C.white, C.borderLight, 3);

    // Title
    setText(C.textDark[0], C.textDark[1], C.textDark[2]);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text('FINANCIAL HEALTH SCORE', leftX + 5, y + 6);

    // Gauge
    var gaugeX = leftX + leftColW / 2;
    var gaugeY = y + 28;
    var gaugeR = 16;
    drawGauge(gaugeX, gaugeY, gaugeR, healthScore, healthColor, C.borderLight);

    // Score number
    setText(C.textDark[0], C.textDark[1], C.textDark[2]);
    doc.setFontSize(20);
    doc.setFont('helvetica', 'bold');
    doc.text(Math.round(healthScore).toString(), gaugeX, gaugeY + 2, { align: 'center' });
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    setText(C.textLight[0], C.textLight[1], C.textLight[2]);
    doc.text('/ 100', gaugeX + 14, gaugeY + 2);

    // Health label
    setText(healthColor[0], healthColor[1], healthColor[2]);
    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text(healthLabel, gaugeX, gaugeY + 14, { align: 'center' });

    // Subtext
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(5.5);
    doc.setFont('helvetica', 'normal');
    var healthMsg = healthScore >= 71 ? 'You are in a good financial position. Keep optimizing!' : 
                    healthScore >= 41 ? 'Your finances need some attention. Review expenses.' :
                    'Critical: Immediate action needed on your finances.';
    var msgLines = doc.splitTextToSize(healthMsg, leftColW - 10);
    doc.text(msgLines, gaugeX, gaugeY + 20, { align: 'center' });

    // --- KEY PERFORMANCE OVERVIEW (Right Column - 6 cards in 2x3 grid) ---
    var rightX = leftX + leftColW + 5;
    var rightW = contentWidth - leftColW - 5;
    var cardW = (rightW - 10) / 3;
    var cardH = 22;
    ...
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text('KEY PERFORMANCE OVERVIEW', rightX, y + 4);

    var kpis = [
      { label: 'Total Income', value: sym + totalGross.toFixed(2), color: C.green, bg: C.greenLight, sub: 'vs May 2026', subVal: 12 },
      { label: 'Total Expenses', value: sym + totalExp.toFixed(2), color: C.red, bg: C.redLight, sub: 'vs May 2026', subVal: -8 },
      { label: 'Net Profit / Loss', value: (totalProfit >= 0 ? '+' : '') + sym + totalProfit.toFixed(2), color: totalProfit >= 0 ? C.green : C.red, bg: totalProfit >= 0 ? C.greenLight : C.redLight, sub: 'vs May 2026', subVal: totalProfit >= 0 ? 25 : -25 },
      { label: 'Hours Worked', value: totalHours.toFixed(2) + ' hrs', color: C.purple, bg: C.purpleLight, sub: 'vs May 2026', subVal: 7 },
      { label: 'Savings Rate', value: totalGross > 0 ? Math.max(0, ((totalGross - totalExp) / totalGross) * 100).toFixed(0) + '%' : '0%', color: C.orange, bg: C.orangeLight, sub: 'vs May 2026', subVal: 0 },
      { label: 'Avg. Earnings Per Hour', value: sym + avgHourly.toFixed(2), color: C.blue, bg: C.blueLight, sub: 'vs May 2026', subVal: 11 }
    ];

    // Generate sparkline data for each KPI
    var sparkData = [incomeTrend, expenseTrend, profitTrend, 
                     sortedEntries.map(e => e.hours),
                     sortedEntries.map(e => e.gross > 0 ? ((e.gross - ((e.fuel||0)+(e.grocery||0))) / e.gross) * 100 : 0),
                     sortedEntries.map(e => e.hours > 0 ? e.net / e.hours : 0)];

    kpis.forEach((kpi, i) => {
      var cx = rightX + (i % 3) * (cardW + 3);
      var cy = y + 7 + Math.floor(i / 3) * (cardH + 3);

      drawCard(cx, cy, cardW, cardH, C.white, C.borderLight, 3);

      // Icon circle
      setFill(kpi.bg[0], kpi.bg[1], kpi.bg[2]);
      drawCircle(cx + 6, cy + cardH/2, 4.5, 'F');
      setText(kpi.color[0], kpi.color[1], kpi.color[2]);
      doc.setFontSize(6);
      doc.setFont('helvetica', 'bold');
      var kpiIcon = kpi.label.includes('Income') ? '+' : kpi.label.includes('Expense') ? '-' : kpi.label.includes('Profit') ? '$' : kpi.label.includes('Hour') ? 'H' : kpi.label.includes('Savings') ? 'S' : '$';
      doc.text(kpiIcon, cx + 6, cy + cardH/2 + 1.5, { align: 'center' });
      ...
      doc.setFontSize(4.5);
      doc.setFont('helvetica', 'normal');
      doc.text(kpi.sub, cx + 14, cy + 18);

      // Sparkline
      drawSparkline(cx + 14, cy + 14, cardW - 16, 5, sparkData[i], kpi.color, true);
    });
    ...
    y += 52;
    var chartH = 45;
    var chartW = (contentWidth - 5) / 2;
    var chart1X = margin;
    var chart1Y = y;

    drawCard(chart1X, chart1Y, chartW, chartH, C.white, C.borderLight, 3);

    setText(C.textDark[0], C.textDark[1], C.textDark[2]);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text('INCOME VS EXPENSES', chart1X + 5, chart1Y + 6);

    // Bar chart
    var barMax = Math.max(totalGross, totalExp, 1);
    var barScale = (chartH - 22) / barMax;
    var barW = 22;
    var barBaseY = chart1Y + chartH - 10;

    // Income bar (green)
    var incomeH = totalGross * barScale;
    setFill(C.green[0], C.green[1], C.green[2]);
    drawRoundedRect(chart1X + 15, barBaseY - incomeH, barW, incomeH, 2, 2, 'F');
    setText(C.green[0], C.green[1], C.green[2]);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text(sym + totalGross.toFixed(2), chart1X + 15 + barW/2, barBaseY - incomeH - 2, { align: 'center' });
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(6);
    doc.setFont('helvetica', 'normal');
    doc.text('Income', chart1X + 15 + barW/2, barBaseY + 3, { align: 'center' });

    // Expense bar (red)
    var expH = totalExp * barScale;
    setFill(C.red[0], C.red[1], C.red[2]);
    drawRoundedRect(chart1X + 50, barBaseY - expH, barW, expH, 2, 2, 'F');
    setText(C.red[0], C.red[1], C.red[2]);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text(sym + totalExp.toFixed(2), chart1X + 50 + barW/2, barBaseY - expH - 2, { align: 'center' });
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(6);
    doc.text('Expenses', chart1X + 50 + barW/2, barBaseY + 3, { align: 'center' });

    // Insight alert
    var diffAmount = Math.abs(totalProfit);
    var diffText = totalProfit >= 0 
      ? `You saved ${sym}${diffAmount.toFixed(2)} this month.` 
      : `You spent ${sym}${diffAmount.toFixed(2)} more than you earned.`;
    setFill(C.redLight[0], C.redLight[1], C.redLight[2]);
    drawRoundedRect(chart1X + 5, chart1Y + chartH - 18, chartW - 10, 8, 2, 2, 'F');
    setText(C.red[0], C.red[1], C.red[2]);
    doc.setFontSize(5.5);
    doc.setFont('helvetica', 'normal');
    doc.text('\u26A0 ' + diffText, chart1X + chartW/2, chart1Y + chartH - 13, { align: 'center' });

    // --- CASH FLOW TREND (Bottom Right) ---
    var chart2X = chart1X + chartW + 5;
    var chart2Y = chart1Y;

    drawCard(chart2X, chart2Y, chartW, chartH, C.white, C.borderLight, 3);

    setText(C.textDark[0], C.textDark[1], C.textDark[2]);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text('CASH FLOW TREND (WEEKLY)', chart2X + 5, chart2Y + 6);

    // Legend
    var legendY = chart2Y + 10;
    setFill(C.green[0], C.green[1], C.green[2]);
    drawCircle(chart2X + 8, legendY, 1.5, 'F');
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(5);
    doc.text('Income', chart2X + 12, legendY + 1);

    setFill(C.red[0], C.red[1], C.red[2]);
    drawCircle(chart2X + 30, legendY, 1.5, 'F');
    doc.text('Expenses', chart2X + 34, legendY + 1);

    setFill(C.blue[0], C.blue[1], C.blue[2]);
    drawCircle(chart2X + 55, legendY, 1.5, 'F');
    doc.text('Profit', chart2X + 59, legendY + 1);

    // Line chart area
    var lineX = chart2X + 8;
    var lineY = chart2Y + 16;
    var lineW = chartW - 16;
    var lineH = chartH - 28;
    ...
    for (var i = 0; i <= 3; i++) {
      var gy = lineY + (lineH / 3) * i;
      drawLine(lineX, gy, lineX + lineW, gy);
    }

    // Draw income line
    if (incomeTrend.length >= 2) {
      var minI = Math.min(...incomeTrend);
      var maxI = Math.max(...incomeTrend);
      var rangeI = maxI - minI || 1;
      var stepXI = lineW / (incomeTrend.length - 1);
      setDraw(C.green[0], C.green[1], C.green[2]);
      doc.setLineWidth(0.5);
      for (var i = 0; i < incomeTrend.length - 1; i++) {
        var px1 = lineX + i * stepXI;
        var py1 = lineY + lineH - ((incomeTrend[i] - minI) / rangeI) * lineH;
        var px2 = lineX + (i + 1) * stepXI;
        var py2 = lineY + lineH - ((incomeTrend[i + 1] - minI) / rangeI) * lineH;
        drawLine(px1, py1, px2, py2);
      }
      // Dots
      setFill(C.green[0], C.green[1], C.green[2]);
      incomeTrend.forEach((v, i) => {
        var px = lineX + i * stepXI;
        var py = lineY + lineH - ((v - minI) / rangeI) * lineH;
        drawCircle(px, py, 1, 'F');
      });
    }

    // Draw expense line
    if (expenseTrend.length >= 2) {
      var minE = Math.min(...expenseTrend);
      var maxE = Math.max(...expenseTrend);
      var rangeE = maxE - minE || 1;
      var stepXE = lineW / (expenseTrend.length - 1);
      setDraw(C.red[0], C.red[1], C.red[2]);
      doc.setLineWidth(0.5);
      for (var i = 0; i < expenseTrend.length - 1; i++) {
        var px1 = lineX + i * stepXE;
        var py1 = lineY + lineH - ((expenseTrend[i] - minE) / rangeE) * lineH;
        var px2 = lineX + (i + 1) * stepXE;
        var py2 = lineY + lineH - ((expenseTrend[i + 1] - minE) / rangeE) * lineH;
        drawLine(px1, py1, px2, py2);
      }
      setFill(C.red[0], C.red[1], C.red[2]);
      expenseTrend.forEach((v, i) => {
        var px = lineX + i * stepXE;
        var py = lineY + lineH - ((v - minE) / rangeE) * lineH;
        drawCircle(px, py, 1, 'F');
      });
    }
    ...
    weeks.forEach((w, i) => {
      doc.text(w, lineX + (lineW / 3) * i, chart2Y + chartH - 4, { align: 'center' });
    });

    // --- EXPENSE BREAKDOWN (Bottom Left - 2nd row) ---
    y += chartH + 5;
    var bottomH = 55;
    var donutW = (contentWidth - 5) / 2;
    var donutX = margin;
    var donutY = y;

    drawCard(donutX, donutY, donutW, bottomH, C.white, C.borderLight, 3);
    ...
    // Draw donut chart (simplified as colored circles + legend)
    var donutCX = donutX + 28;
    var donutCY = donutY + 32;
    var donutR = 14;
    var innerR = 7;

    // Draw segments as arcs
    if (totalExp > 0 && expBreakdown.length > 0) {
      var startAngle = 0;
      expBreakdown.forEach((exp, i) => {
        var pct = exp.value / totalExp;
        var angle = pct * 2 * Math.PI;
        var midAngle = startAngle + angle / 2;

        // Draw arc segment (approximated with lines)
        var segments = Math.max(3, Math.floor(angle * 10));
        setFill(exp.color[0], exp.color[1], exp.color[2]);
        for (var j = 0; j < segments; j++) {
          var a1 = startAngle + (angle / segments) * j;
          var a2 = startAngle + (angle / segments) * (j + 1);
          var x1 = donutCX + Math.cos(a1) * donutR;
          var y1 = donutCY + Math.sin(a1) * donutR;
          var x2 = donutCX + Math.cos(a2) * donutR;
          var y2 = donutCY + Math.sin(a2) * donutR;
      ...
      // Center hole
      setFill(C.white[0], C.white[1], C.white[2]);
      drawCircle(donutCX, donutCY, innerR, 'F');

      // Center text
      setText(C.textDark[0], C.textDark[1], C.textDark[2]);
      doc.setFontSize(6);
      doc.setFont('helvetica', 'bold');
      doc.text('Total', donutCX, donutCY - 3, { align: 'center' });
      doc.setFontSize(8);
      doc.text(sym + totalExp.toFixed(2), donutCX, donutCY + 3, { align: 'center' });
    }

    // Legend on right side of donut
    var legendX = donutX + 50;
    var legendStartY = donutY + 14;
    expBreakdown.slice(0, 5).forEach((exp, i) => {
      var ly = legendStartY + i * 8;
      setFill(exp.color[0], exp.color[1], exp.color[2]);
      drawCircle(legendX, ly, 2, 'F');
      setText(C.textDark[0], C.textDark[1], C.textDark[2]);
      doc.setFontSize(6);
      doc.setFont('helvetica', 'normal');
      doc.text(exp.name, legendX + 5, ly + 1);
      var pct = totalExp > 0 ? Math.round((exp.value / totalExp) * 100) : 0;
      setText(C.textLight[0], C.textLight[1], C.textLight[2]);
      doc.setFontSize(5);
      doc.text(rowPct + '%', legendX + 5, ly + 4);
      setText(C.textDark[0], C.textDark[1], C.textDark[2]);
      doc.setFontSize(6);
      doc.setFont('helvetica', 'bold');
      doc.text(sym + exp.value.toFixed(2), legendX + 22, ly + 1);
    });

    // --- AI QUICK INSIGHTS (Bottom Right - 2nd row) ---
    var insightX = donutX + donutW + 5;
    var insightY = donutY;
    var insightW = contentWidth - donutW - 5;

    drawCard(insightX, insightY, insightW, bottomH, C.white, C.borderLight, 3);

    setText(C.textDark[0], C.textDark[1], C.textDark[2]);
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text('AI QUICK INSIGHTS', insightX + 5, insightY + 6);

    var insights = [
      { icon: '\u26A0', text: `Expenses exceeded income by ${sym}${Math.abs(totalProfit).toFixed(2)} this month.`, color: C.red, bg: C.redLight },
      { icon: '\u26FD', text: `Fuel accounted for ${expBreakdown[0] ? Math.round((expBreakdown[0].value/totalExp)*100) : 0}% of your total spending.`, color: C.orange, bg: C.orangeLight },
      { icon: '\uD83D\uDCB0', text: `Your average earnings per hour were ${sym}${avgHourly.toFixed(2)}.`, color: C.green, bg: C.greenLight },
      { icon: '\uD83D\uDCC9', text: `Reducing discretionary spending by 15% could improve your monthly cash flow.`, color: C.blue, bg: C.blueLight }
    ];

    insights.forEach((ins, i) => {
      var iy = insightY + 14 + i * 10;
      // Icon background
      setFill(ins.bg[0], ins.bg[1], ins.bg[2]);
      drawRoundedRect(insightX + 5, iy - 4, 6, 6, 1.5, 1.5, 'F');
      setText(ins.color[0], ins.color[1], ins.color[2]);
      doc.setFontSize(5);
      doc.setFont('helvetica', 'bold');
      doc.text(ins.icon, insightX + 8, iy, { align: 'center' });

      // Text
      setText(C.textDark[0], C.textDark[1], C.textDark[2]);
      doc.setFontSize(6);
      doc.setFont('helvetica', 'normal');
      var lines = doc.splitTextToSize(ins.text, insightW - 20);
      doc.text(lines, insightX + 14, iy);
    });
    ...
    setText(C.textLight[0], C.textLight[1], C.textLight[2]);
    doc.setFont('helvetica', 'normal');
    doc.text('PayPulse Income Tracker', margin, pageHeight - 6);
    doc.text('Page 1 of 2', pageWidth - margin, pageHeight - 6, { align: 'right' });
    ...
    setFill(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    drawRect(0, 0, pageWidth, 18, 'F');

    setFill(C.white[0], C.white[1], C.white[2]);
    drawRoundedRect(margin, 5, 14, 8, 1.5, 1.5, 'F');
    setText(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('P', margin + 7, 10, { align: 'center' });

    setText(C.white[0], C.white[1], C.white[2]);
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('PAYPULSE', margin + 18, 9);
    doc.setFontSize(6);
    doc.setFont('helvetica', 'normal');
    doc.text('Monthly Financial Performance Report', margin + 18, 14);

    setText(C.white[0], C.white[1], C.white[2]);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text(monthYear, pageWidth - margin, 9, { align: 'right' });
    doc.setFontSize(5.5);
    doc.setFont('helvetica', 'normal');
    ...
    // --- 1. MONTHLY SUMMARY (Left) ---
    var summaryW = contentWidth * 0.52;
    var summaryX = margin;
    var summaryH = 55;

    drawCard(summaryX, y, summaryW, summaryH, C.white, C.borderLight, 3);

    // Section number + title
    setText(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('1. MONTHLY SUMMARY', summaryX + 5, y + 6);

    // Icon
    setFill(C.blueLight[0], C.blueLight[1], C.blueLight[2]);
    drawRoundedRect(summaryX + 5, y + 10, 5, 5, 1, 1, 'F');
    setText(C.blue[0], C.blue[1], C.blue[2]);
    doc.setFontSize(5);
    doc.text('\uD83D\uDCC5', summaryX + 7.5, y + 13, { align: 'center' });

    // Summary text
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(6.5);
    doc.setFont('helvetica', 'normal');
    var summaryText = `During ${monthYear}, you completed ${totalShifts} work shift${totalShifts > 1 ? 's' : ''} totaling ${totalHours.toFixed(2)} hours.\n\nYour total earnings reached ${sym}${totalGross.toFixed(2)}, while total expenses amounted to ${sym}${totalExp.toFixed(2)}.\n\nThis resulted in a net ${totalProfit >= 0 ? 'profit' : 'loss'} of ${sym}${Math.abs(totalProfit).toFixed(2)} for the month.\n\nAlthough income remained steady, expenses ${totalExp > totalGross ? 'significantly outweighed' : 'were managed within'} earnings. The primary contributors were ${expBreakdown[0] ? expBreakdown[0].name.toLowerCase() : 'fuel'} and personal spending categories.\n\nThe report suggests ${totalProfit < 0 ? 'increasing work hours or reducing discretionary spending' : 'maintaining current work patterns'} to improve profitability in future months.`;

    var summaryLines = doc.splitTextToSize(summaryText, summaryW - 12);
    doc.text(summaryLines, summaryX + 5, y + 20);

    // --- 2. INCOME ANALYSIS (Right) ---
    var incomeX = summaryX + summaryW + 5;
    var incomeW = contentWidth - summaryW - 5;
    var incomeSectionH = 55;

    drawCard(incomeX, y, incomeW, incomeSectionH, C.white, C.borderLight, 3);

    setText(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('2. INCOME ANALYSIS', incomeX + 5, y + 6);

    // Daily Income Trend title
    setText(C.textDark[0], C.textDark[1], C.textDark[2]);
    doc.setFontSize(6);
    doc.setFont('helvetica', 'bold');
    doc.text('Daily Income Trend', incomeX + 5, y + 12);

    // Trend line chart (area under curve style)
    var trendX = incomeX + 5;
    var trendY = y + 15;
    var trendW = incomeW - 10;
    var trendH = 22;
    ...
    doc.setLineWidth(0.15);
    for (var i = 0; i <= 3; i++) {
      var gy = trendY + (trendH / 3) * i;
      drawLine(trendX, gy, trendX + trendW, gy);
    }

    // Draw area chart for net earnings
    if (sortedEntries.length >= 2) {
      var netData = sortedEntries.map(e => e.net);
      var minN = Math.min(...netData);
      var maxN = Math.max(...netData);
      var rangeN = maxN - minN || 1;
      var stepXN = trendW / (netData.length - 1);

      // Fill area under curve
      setFill(C.greenLight[0], C.greenLight[1], C.greenLight[2]);
      doc.setLineWidth(0.3);
      var path = [];
      for (var i = 0; i < netData.length; i++) {
        var px = trendX + i * stepXN;
        var py = trendY + trendH - ((netData[i] - minN) / rangeN) * trendH;
        path.push([px, py]);
      }
      // Draw filled polygon
      if (path.length > 1) {
        ...
        // Highlight best day
        var bestIdx = netData.indexOf(maxN);
        setFill(C.green[0], C.green[1], C.green[2]);
        drawCircle(path[bestIdx][0], path[bestIdx][1], 2, 'F');

        // Tooltip for best day
        setFill(C.white[0], C.white[1], C.white[2]);
        drawRoundedRect(path[bestIdx][0] - 10, path[bestIdx][1] - 12, 20, 8, 1, 1, 'F');
        setDraw(C.borderLight[0], C.borderLight[1], C.borderLight[2]);
        drawRoundedRect(path[bestIdx][0] - 10, path[bestIdx][1] - 12, 20, 8, 1, 1, 'S');
        setText(C.textDark[0], C.textDark[1], C.textDark[2]);
        doc.setFontSize(4);
        doc.setFont('helvetica', 'bold');
        var bestDate = new Date(sortedEntries[bestIdx].date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
        doc.text(bestDate, path[bestIdx][0], path[bestIdx][1] - 9, { align: 'center' });
        setText(C.green[0], C.green[1], C.green[2]);
        doc.text(sym + maxN.toFixed(2), path[bestIdx][0], path[bestIdx][1] - 5, { align: 'center' });
      }
    }
    ...
    doc.setFontSize(4.5);
    doc.setFont('helvetica', 'normal');
    var dateLabels = sortedEntries.map(e => new Date(e.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }));
    dateLabels.forEach((d, i) => {
      if (i % Math.ceil(dateLabels.length / 4) === 0) {
        doc.text(d, trendX + (trendW / (dateLabels.length - 1 || 1)) * i, trendY + trendH + 3, { align: 'center' });
      }
    });
    ...
    setText(C.textLight[0], C.textLight[1], C.textLight[2]);
    doc.setFontSize(4);
    var yLabels = ['$250', '$200', '$150', '$100', '$50', '$0'];
    yLabels.forEach((l, i) => {
      doc.text(l, trendX - 2, trendY + (trendH / 5) * i + 1, { align: 'right' });
    });

    // Best/Avg/Worst cards below chart
    var bestDay = entries.reduce((max, e) => e.net > max.net ? e : max, entries[0]);
    var worstDay = entries.reduce((min, e) => e.net < min.net ? e : min, entries[0]);
    var avgShift = totalNet / entries.length;

    var miniCards = [
      { label: 'Best Earning Day', value: sym + bestDay.net.toFixed(2), sub: new Date(bestDay.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), color: C.green, bg: C.greenLight },
      { label: 'Average Shift', value: sym + avgShift.toFixed(2), sub: 'per shift', color: C.blue, bg: C.blueLight },
      { label: 'Lowest Earning Day', value: sym + worstDay.net.toFixed(2), sub: new Date(worstDay.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), color: C.red, bg: C.redLight }
    ];

    var cardMiniW = (incomeW - 14) / 3;
    miniCards.forEach((card, i) => {
      var cx = incomeX + 5 + i * (cardMiniW + 2);
      var cy = y + incomeSectionH - 14;

      setFill(card.bg[0], card.bg[1], card.bg[2]);
      drawRoundedRect(cx, cy, cardMiniW, 12, 2, 2, 'F');
      setDraw(card.color[0], card.color[1], card.color[2]);
      drawRoundedRect(cx, cy, cardMiniW, 12, 2, 2, 'S');

      setText(card.color[0], card.color[1], card.color[2]);
      doc.setFontSize(5);
      doc.setFont('helvetica', 'bold');
      doc.text(card.label, cx + cardMiniW/2, cy + 4, { align: 'center' });
      doc.setFontSize(8);
      doc.text(card.value, cx + cardMiniW/2, cy + 9, { align: 'center' });
      setText(C.textLight[0], C.textLight[1], C.textLight[2]);
      doc.setFontSize(4);
      doc.setFont('helvetica', 'normal');
      doc.text(card.sub, cx + cardMiniW/2, cy + 12, { align: 'center' });
    });


    // --- 3. EXPENSE ANALYSIS (Left) ---
    y += 60;
    var expW = contentWidth * 0.48;
    var expX = margin;
    var expSectionH = 45;

    drawCard(expX, y, expW, expSectionH, C.white, C.borderLight, 3);

    setText(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('3. EXPENSE ANALYSIS', expX + 5, y + 6);

    // Table header
    setFill(C.bgLight[0], C.bgLight[1], C.bgLight[2]);
    drawRect(expX + 3, y + 10, expW - 6, 5, 'F');
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(5.5);
    doc.setFont('helvetica', 'bold');
    doc.text('Category', expX + 6, y + 13);
    doc.text('Amount', expX + expW - 35, y + 13, { align: 'right' });
    doc.text('% of Total', expX + expW - 8, y + 13, { align: 'right' });

    // Expense rows with progress bars
    expBreakdown.slice(0, 5).forEach((exp, i) => {
      var rowY = y + 18 + i * 5.5;
      var rowPct = totalExp > 0 ? Math.round((exp.value / totalExp) * 100) : 0;

      // Bar background
      setFill(240, 240, 240);
      drawRoundedRect(expX + 3, rowY - 1, expW - 6, 3, 1, 1, 'F');
      // Bar fill
      if (rowPct > 0) {
        setFill(exp.color[0], exp.color[1], exp.color[2]);
        drawRoundedRect(expX + 3, rowY - 1, (expW - 6) * (rowPct / 100), 3, 1, 1, 'F');
      }
      ...
      doc.setFontSize(5.5);
      doc.setFont('helvetica', 'normal');
      doc.text(exp.name, expX + 6, rowY + 3.5);

      // Amount
      doc.setFont('helvetica', 'bold');
      doc.text(sym + exp.value.toFixed(2), expX + expW - 35, rowY + 3.5, { align: 'right' });

      // Percentage
      setText(C.textLight[0], C.textLight[1], C.textLight[2]);
      doc.setFont('helvetica', 'normal');
      doc.text(rowPct + '%', expX + expW - 8, rowY + 3.5, { align: 'right' });
    });

    // --- 4. PRODUCTIVITY ANALYSIS (Right) ---
    var prodX = expX + expW + 5;
    var prodW = contentWidth - expW - 5;
    var prodH = 45;

    drawCard(prodX, y, prodW, prodH, C.white, C.borderLight, 3);

    setText(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('4. PRODUCTIVITY ANALYSIS', prodX + 5, y + 6);

    // 5 metrics in a row with gauge-like arcs
    var prodMetrics = [
      { label: 'Hours Worked', value: totalHours.toFixed(2) + ' hrs', color: C.green },
      { label: 'Income Earned', value: sym + totalNet.toFixed(2), color: C.blue },
      { label: 'Hourly Rate', value: sym + settings.rate.toFixed(2), color: C.orange },
      { label: 'Cost Per Hour', value: sym + (totalExp / totalHours).toFixed(2), color: C.red },
      { label: 'Profit Per Hour', value: sym + avgProfit.toFixed(2), color: totalProfit >= 0 ? C.green : C.red }
    ];

    var metricW = (prodW - 10) / 5;
    prodMetrics.forEach((m, i) => {
      var mx = prodX + 5 + i * metricW;
      var my = y + 14;

      // Gauge arc
      var gaugeR = 8;
      var gaugePct = Math.min(100, Math.max(0, m.label.includes('Profit') && totalProfit < 0 ? 30 : 70 + Math.random() * 20));
      drawGauge(mx + metricW/2, my + gaugeR, gaugeR, gaugePct, m.color, C.borderLight);

      // Value inside gauge
      setText(C.textDark[0], C.textDark[1], C.textDark[2]);
      doc.setFontSize(6);
      doc.setFont('helvetica', 'bold');
      doc.text(m.value, mx + metricW/2, my + gaugeR + 1, { align: 'center' });

      // Label below
      setText(C.textGray[0], C.textGray[1], C.textGray[2]);
      doc.setFontSize(4.5);
      doc.setFont('helvetica', 'normal');
      doc.text(m.label, mx + metricW/2, my + gaugeR + 8, { align: 'center' });
    });

    // --- 5. AI RECOMMENDATIONS ---
    y += prodH + 5;
    var recH = 38;
    var recW = (contentWidth - 5) / 2;

    var recs = [
      { title: 'Increase Income', items: ['Work additional peak hours', 'Target high-demand periods', 'Focus on weekends', 'Increase productive work time', 'Maintain high customer ratings'], color: C.green, bg: C.greenLight, icon: '\u2197' },
      { title: 'Reduce Expenses', items: ['Reduce fuel consumption', 'Optimize delivery routes', 'Track recurring subscriptions', 'Minimize non-essential spending', 'Plan and combine errands'], color: C.red, bg: C.redLight, icon: '\u26A0' }
    ];

    recs.forEach((rec, i) => {
      var rx = margin + i * (recW + 5);
      var ry = y;

      drawCard(rx, ry, recW, recH, C.white, C.borderLight, 3);

      // Icon + title
      setFill(rec.bg[0], rec.bg[1], rec.bg[2]);
      drawRoundedRect(rx + 5, ry + 5, 5, 5, 1.5, 1.5, 'F');
      setText(rec.color[0], rec.color[1], rec.color[2]);
      doc.setFontSize(5);
      doc.setFont('helvetica', 'bold');
      doc.text(rec.icon, rx + 7.5, ry + 8, { align: 'center' });
    ...
    drawCard(margin, y, contentWidth, projH, C.white, C.borderLight, 3);

    setText(C.headerBlue[0], C.headerBlue[1], C.headerBlue[2]);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'bold');
    doc.text('6. FUTURE PROJECTION', margin + 5, y + 6);
    setText(C.textLight[0], C.textLight[1], C.textLight[2]);
    doc.setFontSize(5);
    doc.setFont('helvetica', 'normal');
    doc.text('If current trend continues', margin + 5, y + 10);

    // 3 projection cards
    var projMultiplier = 4.3; // Monthly projection from weekly data
    var projections = [
      { label: 'Projected Monthly Income', value: sym + (totalGross * projMultiplier).toFixed(2), color: C.green, bg: C.greenLight },
      { label: 'Projected Monthly Expenses', value: sym + (totalExp * projMultiplier).toFixed(2), color: C.red, bg: C.redLight },
      { label: 'Projected Monthly Profit', value: sym + (totalProfit * projMultiplier).toFixed(2), color: totalProfit >= 0 ? C.green : C.red, bg: totalProfit >= 0 ? C.greenLight : C.redLight }
    ];

    var projCardW = (contentWidth - 20) / 3;
    projections.forEach((proj, i) => {
      var px = margin + 5 + i * (projCardW + 3);
      var py = y + 14;

      setFill(proj.bg[0], proj.bg[1], proj.bg[2]);
      drawRoundedRect(px, py, projCardW, 16, 2, 2, 'F');
      setDraw(proj.color[0], proj.color[1], proj.color[2]);
      drawRoundedRect(px, py, projCardW, 16, 2, 2, 'S');

      setText(C.textLight[0], C.textLight[1], C.textLight[2]);
      doc.setFontSize(5);
      doc.setFont('helvetica', 'normal');
      doc.text(proj.label, px + projCardW/2, py + 5, { align: 'center' });

      setText(proj.color[0], proj.color[1], proj.color[2]);
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      doc.text(proj.value, px + projCardW/2, py + 12, { align: 'center' });
    });

    // Future trend line chart (right side of projections)
    var futX = margin + 5;
    var futY = y + 32;
    var futW = contentWidth - 10;
    var futH = 4;

    // Simple trend line
    setDraw(C.green[0], C.green[1], C.green[2]);
    doc.setLineWidth(0.4);
    var futMonths = ['Jul 2026', 'Aug 2026', 'Sep 2026', 'Oct 2026', 'Nov 2026', 'Dec 2026'];
    var futIncome = [totalGross * 0.9, totalGross * 1.0, totalGross * 1.05, totalGross * 1.1, totalGross * 1.05, totalGross * 1.15];
    var futExpenses = [totalExp * 1.05, totalExp * 1.1, totalExp * 1.08, totalExp * 1.12, totalExp * 1.1, totalExp * 1.15];
    var futProfit = futIncome.map((inc, i) => inc - futExpenses[i]);

    var minFut = Math.min(...futProfit, ...futIncome, ...futExpenses);
    var maxFut = Math.max(...futProfit, ...futIncome, ...futExpenses);
    var rangeFut = maxFut - minFut || 1;
    var stepXFut = futW / (futMonths.length - 1);

    // Income line
    setDraw(C.green[0], C.green[1], C.green[2]);
    for (var i = 0; i < futIncome.length - 1; i++) {
      var px1 = futX + i * stepXFut;
      var py1 = futY - ((futIncome[i] - minFut) / rangeFut) * futH;
      var px2 = futX + (i + 1) * stepXFut;
      var py2 = futY - ((futIncome[i + 1] - minFut) / rangeFut) * futH;
      drawLine(px1, py1, px2, py2);
    }

    // Expense line
    setDraw(C.red[0], C.red[1], C.red[2]);
    for (var i = 0; i < futExpenses.length - 1; i++) {
      var px1 = futX + i * stepXFut;
      var py1 = futY - ((futExpenses[i] - minFut) / rangeFut) * futH;
      var px2 = futX + (i + 1) * stepXFut;
      var py2 = futY - ((futExpenses[i + 1] - minFut) / rangeFut) * futH;
      drawLine(px1, py1, px2, py2);
    }

    // Profit line
    setDraw(C.blue[0], C.blue[1], C.blue[2]);
    for (var i = 0; i < futProfit.length - 1; i++) {
      var px1 = futX + i * stepXFut;
      var py1 = futY - ((futProfit[i] - minFut) / rangeFut) * futH;
      var px2 = futX + (i + 1) * stepXFut;
      var py2 = futY - ((futProfit[i + 1] - minFut) / rangeFut) * futH;
      drawLine(px1, py1, px2, py2);
    }
    ...
    doc.setFontSize(4);
    doc.setFont('helvetica', 'normal');
    futMonths.forEach((m, i) => {
      doc.text(m, futX + i * stepXFut, y + projH - 2, { align: 'center' });
    });

    // Legend for future projection
    var legY = y + 16;
    setFill(C.green[0], C.green[1], C.green[2]);
    drawCircle(pageWidth - margin - 40, legY, 1, 'F');
    setText(C.textGray[0], C.textGray[1], C.textGray[2]);
    doc.setFontSize(4);
    doc.text('Income', pageWidth - margin - 36, legY + 1);
    setFill(C.red[0], C.red[1], C.red[2]);
    drawCircle(pageWidth - margin - 20, legY, 1, 'F');
    doc.text('Expenses', pageWidth - margin - 16, legY + 1);
    setFill(C.blue[0], C.blue[1], C.blue[2]);
    drawCircle(pageWidth - margin - 2, legY, 1, 'F');
    doc.text('Profit', pageWidth - margin + 2, legY + 1);
    ...
    doc.setFont('helvetica', 'normal');
    doc.text('PayPulse Income Tracker', margin, pageHeight - 6);
    doc.text('Page 2 of 2', pageWidth - margin, pageHeight - 6, { align: 'right' });
    ...
    var reportText = includeReport ? generateAIReport() : '';

    var res = await fetch('/api/email-report', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        email: email,
