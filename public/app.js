...
async function exportPDF() {
  ...
  try {
    ...
    doc.text('PAYPULSE', margin, 18);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    doc.text('Income Tracker Report', margin, 24);

    doc.setTextColor(100, 100, 100);
    doc.setFontSize(9);
    doc.text(`Generated: ${new Date().toLocaleDateString()}`, pageWidth - margin, 14, { align: 'right' });
    doc.text(`User: ${currentUser ? currentUser.username : 'Guest'}`, pageWidth - margin, 19, { align: 'right' });
    doc.text(`Currency: ${currency}`, pageWidth - margin, 24, { align: 'right' });
    ...
    doc.roundedRect(margin, yPos, 42, 22, 3, 3, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8);
    doc.text('GROSS', margin + 3, yPos + 7);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text(sym + totalGross.toFixed(2), margin + 3, yPos + 17);

    doc.setFillColor(0, 230, 118);
    doc.roundedRect(margin + 46, yPos, 42, 22, 3, 3, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('NET', margin + 49, yPos + 7);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text(sym + totalNet.toFixed(2), margin + 49, yPos + 17);

    doc.setFillColor(255, 145, 0);
    doc.roundedRect(margin + 92, yPos, 42, 22, 3, 3, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('HOURS', margin + 95, yPos + 7);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text(totalHours.toFixed(2), margin + 95, yPos + 17);

    let profitColor = totalProfit >= 0 ? [0, 230, 118] : [255, 23, 68];
    doc.setFillColor(...profitColor);
    doc.roundedRect(margin + 138, yPos, 42, 22, 3, 3, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('PROFIT', margin + 141, yPos + 7);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text(sym + totalProfit.toFixed(2), margin + 141, yPos + 17);
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('AI ANALYSIS REPORT', margin, yPos);
    yPos += 6;

    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    let reportLines = doc.splitTextToSize(report, contentWidth);
    ...
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('SHIFT HISTORY', margin, yPos);
    yPos += 4;
    ...
    doc.autoTable({
      startY: yPos,
      head: [['Date', 'Time', 'Hrs', 'Gross', 'Tax', 'Net', 'Profit']],
      body: tableData,
      theme: 'striped',
      headStyles: { 
        fillColor: [41, 98, 255], 
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(`Page ${i} of ${pageCount} | PayPulse Income Tracker`, pageWidth / 2, pageHeight - 8, { align: 'center' });
      doc.text(`Confidential - ${new Date().getFullYear()}`, margin, pageHeight - 8);
    }
