...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    // Ensure user object has all expected fields (backward compatibility)
    user.email = user.email || user.username || '';
    user.custom_labels = user.custom_labels || '{}';
    user.show_deliveries = user.show_deliveries !== undefined ? user.show_deliveries : 1;
    user.name = user.name || '';
